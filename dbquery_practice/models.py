...
class Student(models.Model):
    profile_image = models.ImageField(upload_to=image_content, verbose_name="student_image", validators=[FileExtensionValidator(allowed_extensions=['jpg', 'png', 'jpeg'])])
    first_name = models.CharField(max_length=256, db_index=True)
    last_name = models.CharField(max_length=256, db_index=True)
    standard = models.ForeignKey('Standard', related_name="student_standard", on_delete=models.SET_NULL, null=True)
    hobby = models.CharField(max_length=256, db_index=True)
    email = models.EmailField(max_length=256, unique=True, db_index=True)
    contect_no = models.CharField(max_length=10, unique=True, help_text="enter 10 digit number", db_index=True)
